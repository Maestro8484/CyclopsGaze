...
| Display(s) | 1× GC9A01A module, price not verified this session (~$8-12 typical) | — |
| MCU + 2 displays, integrated | — | ESP32-S3-DualEye-LCD-1.28 — **~$16-18** |
| Sensor | DFRobot SEN0626 — **$14.90** | DFRobot SEN0626 — **$14.90** |
| **Single-eye total** | **~$55-58** | **~$31-33** (and you get the second eye anyway) |
...
**two onboard 1.28in 240×240 GC9A01 round IPS panels**. ~$16-18.
...
- **It deletes the failure mode this repo actually keeps hitting.** Every bench failure in
  [ENGINEERING_LOG.md](ENGINEERING_LOG.md) has been *wiring*, not code: the I²C/UART DIP switch
...
| `sensors/SEN0626Sensor.*` | 285 | **3 blockers**, all listed below. Contract-sensitive — § 7. |
...
**Prior art, do not hand-roll:** LovyanGFX (best S3 story — DMA, sprite/partial-update support) or
TFT_eSPI. Both have GC9A01A support. A hand-written SPI driver would be inventing over a mature
...
Plus a 240×240×16bpp framebuffer = **115,200 B per eye**.
...
- Framebuffers (2 × 115,200 = 225 KB) → **PSRAM**. Sequential, DMA'd out to the panels — the access
  pattern PSRAM is actually good at.

This is exactly why the recommendation is an **R8 part and not a bare N8/no-PSRAM devkit**: PSRAM does
not fix the LUT problem (it is off-chip and cached through the same path — putting LUTs there could be
*worse* than flash), but it frees the internal SRAM that does.
...
two large textures (221 KB) in flash — their access has real spatial locality, since `tx` tracks angle
...
3. Run the **real** `renderEye()` inner loop over a full 240×240 frame with the real access pattern.
